...
img = ImageTk.PhotoImage(Image.open(path+str(0) + "_an.png"))
img2 = ImageTk.PhotoImage(Image.open(path+str(0) + ".png"))
...
def show_image(it_num, path):
	global root
	img = ImageTk.PhotoImage(Image.open(path+run_id+str(it_num) + "_an.png"))
	img2 = ImageTk.PhotoImage(Image.open(path+run_id+str(it_num) + ".png"))
...
def highlight(it_num, mat, zc, H, W, green, tot_reg, b_x, b_y):
	im = mat
	pixelMap = im.load()
...
#	im.show()
	global path
	global run_id
	img.save(path+run_id+str(it_num) + "_an.png")
